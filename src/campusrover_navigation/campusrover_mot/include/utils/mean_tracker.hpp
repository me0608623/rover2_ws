#ifndef MEAN_TRACKER_HPP
#define MEAN_TRACKER_HPP
...
class MeanTracker{
    public:
        ...
        MeanTracker(MatrixXd _points, double _time, int _uid, TrackerParam _param);
...
MatrixXd MeanTracker::cov_compute(MatrixXd mat){
    MatrixXd centered = mat.rowwise() - mat.colwise().mean();

    return (centered.transpose() * centered) / double(mat.rows() - 1);
}

void MeanTracker::judge_status(){
...
void MeanTracker::record_history(MatrixXd mat){
...
std::pair<BoundaryPt, BoundaryPt> MeanTracker::fov_range_compute(MatrixXd &pts, MatrixXd center_pt, int numerator, int denominator){
    int size = pts.rows();
...
MeanTracker::MeanTracker(MatrixXd _points, double _time, int _uid, TrackerParam _param){
    points = _points;
    born_time = _time;
    update_time = _time;
    uid = _uid;
...
void MeanTracker::update(double dt, bool is_detected){
    kf->H(0, 2) = dt;
    kf->H(1, 3) = dt;
    ...
    if(is_detected){
        mean_2X1 = points.leftCols(2).colwise().mean().transpose();
        mean_3X1 = points.colwise().mean().transpose();
        record_history(mean_2X1);

        covariance_2X2 = cov_compute(points.leftCols(2));

        kf->update(mean_2X1);
        
        speed_2X1 = kf->x.bottomRows(2);
...
void MeanTracker::predict(double dt, Matrix<double, 2, 1> &mean_out, Matrix<double, 2, 2> &cov_out, Matrix<double, 2, 1> &speed_out){
    kf->F(0, 2) = dt;
    kf->F(1, 3) = dt;
    ...
    cov_out = covariance_2X2;

    speed_out = pre.bottomRows(2);
}
